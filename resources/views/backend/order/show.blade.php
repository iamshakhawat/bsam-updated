...
<h5 class="card-header">Order       <a href="{{route('order.pdf',$order->id)}}"
   class=" btn btn-sm btn-primary shadow-sm float-right"><i class="fas fa-download fa-sm text-white-50"></i> Generate PDF</a>
  </h5>
  <div class="card-body">
    @if($order)
    ...
    @php
        $orderproducts =  \App\Models\OrderedProduct::where('user_id',$order->user_id)->where('order_id',$order->id)->get();
        @endphp
      <tr>
        <th>Image</th>
        <th>Title</th>
